...
func _on_TextureGenerator_output_generated(image, info):
	if not _applying:
		...
		var img_format = data.get_channel_format(info.maptype)
		var random_pattern = _terrain.get_random_pattern()
		
		# random pattern is for the first call empty => fill entries with images
		if (len(random_pattern) <= info.maptype):
			# print("resizing random_pattern")
			random_pattern.resize(info.maptype)
			var num_to_add = info.maptype + 1 - len(random_pattern)
			for i in range(0, num_to_add):
				random_pattern.append(null)

		if random_pattern[info.maptype] == null:
			# print("create image in random pattern")
			var rnd_pattern_height = Image.new()
			rnd_pattern_height.create(resolution, resolution, false, img_format)
			random_pattern[info.maptype] = rnd_pattern_height

		image.convert(img_format)

		random_pattern[info.maptype].blit_rect(image, \
